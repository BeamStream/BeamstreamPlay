		...
		function(BaseView, SelectBox, Errors) {
			...
			FormView = BaseView
					.extend({
						...
						_onBlur: function(e){this.onBlur(e);},
						onBlur: function(e){
							...
							if($(e.target).parents('.ui-custom')[0])
								$($(e.target).parents('.ui-custom')[0]).removeClass('focus');
						...
						_onKeyPress: function(e){this.onKeyPress(e);},
						...
						_onKeyUp: function(e){
							var pNode = $(e.target).parents('.ui-custom')[0], txt=$(pNode).find('.select-txt')[0];
							if(txt)
								txt.innerHTML = $(e.target).find("option:selected").text();
							this.onKeyUp(e);
						},
						onKeyUp : function(e) {
						...
						onChange : function(e) { 
							...
							this.setValue(this.data.models[0], name.split('.'), value, _.indexOf(this.$('[name="' + name + '"]'), e.target));
							
							if(e.target.nodeName=="INPUT" && e.target.getAttribute('type').toLowerCase()=='radio'){
							}else{
						...
						_onSave: function(e){this.onSave(e);},
						onSave: function(e){},
						saveForm : function(e, callback) {
							_.bindAll(this, 'success', 'error', 'afterSubmitError',
								'afterSubmitSuccess', 'success', 'error',
								'serverError');
							...
							if (!dataToSave.isValid(true)) return false;
								...
								for (attr in model.attributes) {
									...
									if (attr == p[0]) {
										if (child instanceof Backbone.Model) {
											p.shift();
											this.setValue(model.attributes[attr], p, v)
							...
							if (_.isObject(data)) {
								...
								if (error_msg)
									...
									error_msg = this.$('.content').append(
											'<div class="error_msg_area">'+ data.response + '</div>');
