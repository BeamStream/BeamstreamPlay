 ...
 * info@toobler.com Web site : http://www.toobler.com Created : 07/April/2013
...
define(
		[ 'baseView', 'view/imageListView', 'view/videoListView',
				'view/documentListView', 'view/pdfListView',
				'view/presentationListView', 'view/googleDocListView',
				'../../lib/bootstrap-modal',
				'../../lib/extralib/jquery.shuffle', '../../lib/bootstrap',
				'text!templates/fileItem.tpl', ],
		function(BaseView, ImageListView, VideoListView, DocumentListView,
				PdfListView, PresentationListView, GoogleDocListView,
				BootstrapModal, Shuffle, Bootstrap, FileItemTpl) {

			var FilesOverView;
			FilesOverView = BaseView
					.extend({
						...
						displayPage : function(callback) {
							if ($('#grid').attr('name') == 'allFiles') {
								...
								if (this.data.models[0].attributes.documents) {
									_
											.each(
													this.data.models[0].attributes.documents,
													function(model) {

														var extension = '', fileType = '';

														extension = (model.documentURL)
								...
								if (this.data.models[0].attributes.media) {
									_
											.each(
													this.data.models[0].attributes.media,
													function(model) {
														if (model.mediaUrl != "") {
															fileType = '';
							...
							// $('#grid
							// div.content').append(template(JSON.parse(JSON.stringify(this.data))));
						...
						showPresentationList : function() {
							...
							presentationListView = new PresentationListView({
								el : $('#grid')
							});
						...
						showGoogleDocList : function() {
							...
							googleDocListView = new GoogleDocListView({
								el : $('#grid')
							});
