...
Backbone.Validation = (function(Backbone, _, undefined) {
    ...
    var validateObject = function(view, model, validation, attrs, options, attrPath) {
        ...
        for (changedAttr in attrs) {
        	//console.log(changedAttr, attrs)
        	if(attrs[changedAttr] instanceof Backbone.Model){
        		//console.log('changedAttr', attrs[changedAttr])
        		validateObject(view, attrs[changedAttr], attrs[changedAttr].validation, attrs[changedAttr].toJSON(), options, attrPath)
        	}else{
    ...
    var bindModel = function(view, model, options) {
    	...
    	for (var prop in attrs) {
    	    if(_.isArray(attrs[prop])){}
    	    else if(_.isObject(attrs[prop])){
    	    	_.extend(attrs[prop], mixin(view, options));
    	    }
    	}
