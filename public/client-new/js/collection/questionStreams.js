define(
		[ 'collection/baseCollection', '../model/question', '../model/stream' ],
		function(BaseCollection, Question, Stream) {
			var QuestionStreams = BaseCollection
					.extend({
						model : Question,
						objName : 'questionStreams',

						initialize : function() {
							this.on('statusChangeModel', this.updateEditStatus,
									this);
							this.on('questionAnsweredModel',
									this.questionAnswered, this);
							this.on('questionModelDelete',
									this.questionDeleted, this);
							this.on('change:questionRock', this.sort, this);
							this.counter = 0;
						},

						// sort models as they're added by how many upvotes they
						// have
						comparator : function(question) {
							return -question.get('question').rockers.length;
						},

						// adds the current onlineUser id and whether a question
						// was asked
						...
						addRockedByUser : function(onlineUserId, streamOwner) {
							...
							this
									.each(function(question) {
										...
										var onlineUserAsked = question
												.get('question').userId.id === onlineUserId ? true
												: false;
										question.set({
										...
										var StreamOwner = streamOwner === onlineUserId ? true
												: false;
										question.set({
										...
										if (checkForId(
												question.get('question').rockers,
												onlineUserId)) {
											question.set('onlineUserRocked',
													true);
										} else {
											question.set('onlineUserRocked',
													false);
