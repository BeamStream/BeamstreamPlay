	      	...
	      	for(i=1; i <= current; i++)
	      	{
		      	...
		      	if($('#graduated-'+i).val()== "attending" || $('#graduated-'+i).val()== "no")
		        {
		      	...
		      	{   
		   		...
		   		school.set({id:i,schoolName: $('#school-name-'+i).val(),assosiatedSchoolId:1,year:{name: $('#year-'+i).val()}, degreeExpected:{name: degreeexp}, major: $('#major-'+i).val(), degree:{name: $('#degreeprogram-'+i).val() }, graduated: $('#graduated-'+i).val(), graduationDate: degdate ,otherDegree: otherDegree});
