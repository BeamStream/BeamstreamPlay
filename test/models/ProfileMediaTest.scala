...
class UserMediaTest extends FunSuite with BeforeAndAfter {
  ...
  test("Get all media for a user") {
    ...
    val UserMediaObj = new UserMedia(new ObjectId, userId, "http://beamstream.com/Neel.png", UserMediaType.Image, true,"",0,List())
  ...
  test("get all media for a user") {
    val userId = User.createUser(user)
    val UserMediaObj1 = new UserMedia(new ObjectId, userId, "http://beamstream.com/Neel.png", UserMediaType.Image, true,"",0,List())
    val UserMediaObj2 = new UserMedia(new ObjectId, userId, "http://beamstream.com/Neel.png", UserMediaType.Image, true,"",0,List())
