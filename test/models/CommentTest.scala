...
class CommentTest extends FunSuite with BeforeAndAfter {
  ...
  test("Get All Comments For A Model") {
    running(FakeApplication()) {
      ...
      val comment = Comment(new ObjectId, "Comment1", new Date, userId.get, user.firstName, user.lastName, 0, Nil, streamId.get)
      val commentId = Comment.createComment(comment)
      val anotherComment = Comment(new ObjectId, "Comment2", new Date, userId.get, user.firstName, user.lastName, 11, Nil, streamId.get)
  ...
  test("Find whether a User is a Rocker of a Comment or not") {
    running(FakeApplication()) {
      ...
      val comment2 = Comment(new ObjectId, "Comment2", new Date, userId.get, user.firstName, user.lastName, 0, List(), streamId.get)
      val commentId2 = Comment.createComment(comment2)
