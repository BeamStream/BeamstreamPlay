...
class StreamControllerTest extends FunSuite with BeforeAndAfter {
  ...
  test("Render Stream page") {
    ...
    running(FakeApplication()) {
      val result = route(FakeRequest(GET, "/stream").withCookies(Cookie("Beamstream", userId.get.toString() + " browsemedia"))).get
      assert(status(result) === 303)
    }
    running(FakeApplication()) {
      val result = route(FakeRequest(GET, "/stream").withCookies(Cookie("Beamstream", userId.get.toString() + " registration"))).get
    ...
    running(FakeApplication()) {
      val result = route(FakeRequest(GET, "/stream").withCookies(Cookie("Beamstream", userId.get.toString() + " ok"))).get
      assert(status(result) === 303)
    }
    ...
    running(FakeApplication()) {
      val result = route(FakeRequest(GET, "/stream").withCookies(Cookie("Beamstream", user2Id.get.toString() + " registration"))).get
      assert(status(result) === 303)
    }
  }
